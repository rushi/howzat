...
from actions.notification import NotificationService, get_notification_service
...
class TestNotify:
    ...
    def test_returns_true_when_disabled(self, test_settings) -> None:
        ...
        with patch("actions.notification.get_settings", return_value=test_settings):
            service = NotificationService()
    ...
    def test_uses_pync_when_available(self, test_settings, mock_pync: MagicMock) -> None:
        ...
        with (
            patch("actions.notification.get_settings", return_value=test_settings),
            patch("actions.notification.HAS_PYNC", True),
        ):
            service = NotificationService()
            result = service.notify("Message")
    ...
    def test_falls_back_to_osascript(self, test_settings, mock_osascript: MagicMock) -> None:
        ...
        with (
            patch("actions.notification.get_settings", return_value=test_settings),
            patch("actions.notification.HAS_PYNC", False),
        ):
            service = NotificationService()
            result = service.notify("Message")
...
class TestGetNotificationService:
    ...
    def test_returns_notification_service(self) -> None:
        ...
        actions.notification._service = None
